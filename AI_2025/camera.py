...
class HandTracker():
    ...
    def gesture_command(self):
        ...
        if self.lm_list:
            ...
            if len(set(self.finger_down(fingers=[4, 8, 12, 16, 20]))) == 1 and self.finger_down(fingers=[4, 12, 16, 20])[0] == True:
                #print("Bedroom Light OFF")
                self.command = "BL0"

            # turn garage light on ("2" with index and middle)
            if abs(self.lm_list[12][2] - self.lm_list[0][2]) >= 200:
                h = self.finger_down(fingers=[20, 16, 4])
                if len(set(h)) == 1 and h[0] == True:
            ...
            if abs(self.lm_list[8][2] - self.lm_list[0][2]) >= 150 and abs(self.lm_list[20][2] - self.lm_list[0][2]) >= 135:
                h = self.finger_down(fingers=[12, 16])
                if len(set(h)) == 1 and h[0] == True:
...
def main():
    ...
    while True:
        ...
        frame = tracker.hand_finder(frame)   # detect & draw landmarks
        tracker.position_finder(frame)       # update landmark list
        tracker.gesture_command()            # print command if detected
